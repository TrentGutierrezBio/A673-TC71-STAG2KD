...
ggplot(A673_PCA, aes(PC1, PC2, color = GENOTYPE)) +
  geom_point(size = 3) +
  ggtitle("PCA Plot for A673 SA2 KO Samples") +
...
ggplot(TC71_PCA, aes(PC1, PC2, color = GENOTYPE)) +
  geom_point(size = 3) +
  ggtitle("PCA Plot for TC71 SA2 KO samples") +
...
EnhancedVolcano(A673_sig_ordered_result,
  ...
  x = "log2FoldChange",
  y = "pvalue",
  title = "Siginifcant DEGs for A673 SA2 KO Cells",
  subtitle = "",
  pointSize = 1.0,
  labSize = 4.0,
...
EnhancedVolcano(TC71_sig_ordered_result,
  ...
  x = "log2FoldChange",
  y = "pvalue",
  title = "Siginifcant DEGs for TC71 SA2 KO Cells",
  subtitle = "",
  pointSize = 1.0,
  labSize = 4.0,
...
overexp_gene_set <- ggVennDiagram(x = list(A673 = A673_overexp_gsea, TC71 = TC71_overexp_gsea)) +
  ggtitle("Overlapping Overexpressed Gene Sets") +
...
underexp_gene_set <- ggVennDiagram(x = list(A673 = A673_underexp_gsea, TC71 = TC71_underexp_gsea)) +
  ggtitle("Overlapping Underexpressed Gene Sets") +
  theme(plot.title = element_text(hjust = 0.5)) +
  scale_color_brewer(palette = "Blues") +
  scale_fill_gradient(low = "#ece7f2", high = "#2b8cbe")
...
overexp_genes_data$TC71 %>%
  datatable(class = "cell-border stripe", caption = htmltools::tags$caption("Table 5: TC71 GSEA Underexpressed Gene Sets", style = "color:black"), rownames = FALSE)
...
underexp_genes_data$TC71 %>%
  datatable(class = "cell-border stripe", caption = htmltools::tags$caption("Table 8: TC71 GSEA Overexpressed Genesets", style = "color:black"), rownames = FALSE)
...
**Any Notable results?**
The most notable results I'm curious are from the GSEA results, The two cell lines A673 and TC71 had the same treatment applied to them and both cell lines seem to be relatively similar to each other both are classified as Ewing Sarcoma cell lines, but the GSEA over- & under- Enrichment Venn Diagrams show that they have very little overlapping enriched gene sets (~7-10%) where the A673 showed the most amount of enrichment for both over- & under- compared to TC71. 
